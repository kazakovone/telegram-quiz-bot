...
QUESTIONS = [
    {
        ...
        "options": [
            "1. Передать идею и смысл, сделать кадр, который что-то говорит.",
            ...
            "5. Рассказать историю, чтобы человек прожил её, глядя на кадры.",
        ],
    },
    {
        ...
        "options": [
            "1. Цвете, композиции, свете, чтобы всё соответствовало моей идее.",
            "2. Настроении человека и атмосфере вокруг.",
            "3. Качестве картинки, технических моментах, структуре работы.",
            "4. Как герои кадра и пространство взаимодействуют друг с другом.",
            "5. О том, какую историю я создаю серией, и в голове вижу ряд кадров, какие ещё нужно отснять.",
        ],
    },
    {
        ...
        "options": [
            ...
            "5. Помочь человеку увидеть себя в новом свете как героя истории.",
        ],
    },
    {
        ...
        "options": [
            "1. Те, где есть ощущение наилучшей передачи твоей идеи.",
            "2. Те, где видно чувство и живые эмоции.",
            "3. Те, где всё выполнено идеально и качественно.",
            "4. Те, где есть воздух, покой и честность момента.",
            "5. Те, которые вместе складываются в рассказ.",
        ],
    },
    {
        ...
        "options": [
            ...
            "4. Как галерея работ с минималистичным воздушным пространством.",
            "5. Как фильм или книга, чтобы человек не просто смотрел, а проживал и чувствовал.",
        ],
    },
    {
        ...
        "options": [
            "1. Немного волнения – ведь это часть души.",
            ...
            "5. Интерес – что почувствуют люди, когда увидят историю целиком.",
        ],
    },
    {
        ...
        "options": [
            "1. Люблю вдохновляться, но делаю по-своему.",
            "2. Беру идеи, если они близки по духу.",
            "3. Следую тем, что приносят клиентов и продажи.",
            "4. Наблюдаю со стороны, не спешу повторять.",
            "5. Предпочитаю timeless стиль – чтобы фото жили дольше, чем мода.",
        ],
    },
    {
        ...
        "options": [
            "1. Закончить проект – хочется довести идею до совершенства.",
            ...
            "5. Структурировать материал – хочется, чтобы всё само сложилось в историю.",
        ],
    },
    {
    ...
    },
    {
        ...
        "options": [
            "1. Вдохновение, желание задуматься.",
            "2. Тепло, принятие.",
            "3. Уверенность, надёжность.",
            "4. Спокойствие, гармонию.",
            "5. Увлечённость.",
        ],
    },
    {
    ...
    },
    {
        ...
        "options": [
            "1. Творческие проекты и развитие своего собственного стиля.",
            "2. Своя аудитория, тёплое сообщество клиентов.",
            "3. Устойчивость, стабильность, заказы.",
            "4. Свобода, больше времени на себя и смысл.",
    ...
    },
    {
        ...
        "options": [
            ...
            "5. «Эта серия – будто я только что посмотрел прекрасный фильм.»",
        ],
    },
    {
        ...
        "options": [
            "1. Чтобы сайт был точным отражением моего внутреннего мира.",
            "2. Живое ощущение присутствия и тепла.",
            "3. Понятная структура, логика, кнопки.",
            "4. Простота, воздух, фотографии крупно.",
            "5. Целостное повествование – чтобы всё выстраивалось как история.",
        ],
    },
]
...
TYPE_TEXTS = {
    ...
    5: (
        ...
        "<b>Тексты:</b> как сценарий – с началом, атмосферой и кульминацией; сайт в первую очередь должен вызывать эмоции.\n\n"
        "<b>Фишки:</b> страница «Серии» с описаниями, блог с закулисьем, форма "
...
def build_reply_keyboard(options: list[str]) -> ReplyKeyboardMarkup:
    # Каждая опция — отдельная белая кнопка (ReplyKeyboard)
...
def build_final_buttons() -> InlineKeyboardMarkup:
    # Кнопки под финальным текстом (inline)
...
def build_julia_button() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
...
async def send_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    q = QUESTIONS[q_index]
    text = f"<b>{q['title']}</b>\n\n{q['question']}"
    keyboard = build_reply_keyboard(q["options"])
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data in ("start_quiz", "restart_quiz"):
        ...
        # Убираем inline-клавиатуру старта
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработка текстовых ответов с клавиатуры."""
    text = (update.message.text or "").strip()

    # Если тест ещё не начат — предлагать /start
    ...
    # Пытаемся вытащить номер ответа (первый символ 1–5)
    if not text or text[0] not in "12345":
        await update.message.reply_text(
            "Пожалуйста, выбери один из вариантов ответа, нажав на кнопку 1–5."
        )
        return
...
async def show_results(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Сначала уберём клавиатуру с ответами
    await chat.send_message(
        "Спасибо, что прошёл(а) тест! Считаю результаты...",
        reply_markup=ReplyKeyboardRemove(),
    )

    if not answers:
        await chat.send_message(
            "Похоже, тест не был пройден. Нажми /start, чтобы попробовать ещё раз."
        )
        return
    ...
    await chat.send_message(
        text=final_text,
        reply_markup=build_final_buttons(),
        parse_mode=ParseMode.HTML,
    )
    ...
    await chat.send_message(
        text=julia_text,
        reply_markup=build_julia_button(),
        parse_mode=ParseMode.HTML,
    )
